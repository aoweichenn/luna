...
- Semantic lowering implementations live under
  ...
  this order: `domain`, `classes/model`, `generics/model`, `callables`,
  ...
  `stmt/{api,labels}`). The same-module files `callables/{storage,bindings,validation}.la`,
  `context/{input,diagnostics,symbols,lookup}.la`,
