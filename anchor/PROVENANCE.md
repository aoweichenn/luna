...
The original anchor set inherited from the archived `m0` branch consisted of
three fixed-point executables, byte-identical to the tools inside the canonical
seed archive:
...
Those executables were freestanding static x86-64 Linux ELF64 files built from
pure Luna sources by the m0 bootstrap pipeline (C23 seed -> stage 1 -> stage 2
-> stage 3, with stage 2 and stage 3 byte-for-byte identical). They contained
no libc, no dynamic loader and no hosted assembler or linker output.

The current directory contains one subsequently promoted multi-command `luna`
executable; the dated entries below record each transition from the original
set. This directory remains the sole trusted binary input of the repository.
Every later toolchain is built from the Luna sources in `library/`, `compiler/`
and `drivers/` using the current anchor, then verified against its own rebuild
