...
bool luna_x86_64_emit_register_allocation(const LunaIrModule *module,
    ...
    if (module == NULL || diagnostics == NULL || output == NULL ||
        ...
        if (diagnostics != NULL) {
            luna_diagnostic_error_plain(
                diagnostics,
                "x86-64 register allocation emission received invalid state");
        }
        return false;
    }
    ...
    const bool allocated = analyzed && luna_x86_64_register_allocate(
                                           &machine_module, &liveness,
                                           &allocation, diagnostics->stream);
    if (analyzed && !allocated) {
        luna_diagnostic_error_plain(diagnostics,
                                    "x86-64 register allocation failed");
    }
    const bool success =
        allocated && luna_x86_64_register_allocation_print(
                         &machine_module, &liveness, &allocation, output);
    if (allocated && !success) {
        luna_diagnostic_error_plain(
            diagnostics,
            "out of memory while printing x86-64 register allocation");
    }
